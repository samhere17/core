(function( $ ) {
	...
	$.fn.buildTable = function (options) {
		...
		if ($(this).is("table")) {
			...
			/* Preparing table */
			if ($.isEmptyObject(settings.header) == true
					&& $.isEmptyObject(settings.data) == true) {
			...
			} else if ($.isEmptyObject(settings.header) == false
					&& $.isEmptyObject(settings.data) == true) {
			...
			} else if ($.isEmptyObject(settings.header) == true
					&& $.isEmptyObject(settings.data) == false) {
	...
	addHeading = function(table, headingArr, settings) {
		...
		$.each(headingArr, function(index,value) {
			heading = heading + '<th class="text-center"><div><span>' + value +'</span>'
			if (settings.sort) {
	...
	addNoRecord = function(table, settings) {
		var body = '<tbody><tr><td class="text-center" colspan="'+settings.header.length+'">No record</td></tr></tbody>'
		table.append(body)
	}
}( jQuery ))