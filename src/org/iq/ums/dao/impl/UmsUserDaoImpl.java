...
public class UmsUserDaoImpl extends BaseDaoImpl<UmsUser> implements UmsUserDao {
	...
	// private static final String UMS_USER_UPDATE_ALL_BY_USER_ACCESS_KEY = "UPDATE UMS_USER SET USERNAME=?, PASSWORD=?,
	// USER_TYPE=?, USER_STATUS=? WHERE USER_ACCESS_KEY = ?";
	...
	private static final String	DELETE_SOFT_BY_USER_ACCESS_KEY		= "UPDATE UMS_USER SET USER_STATUS = "
			+ UserStatus.DELETED.getUserStatusValue() + " WHERE USER_ACCESS_KEY = ?";
	 ...
	 * 
	 * USER_ID INT NOT NULL AUTO_INCREMENT, USER_ACCESS_KEY VARCHAR(50) NOT NULL UNIQUE, USERNAME VARCHAR(50) NOT NULL,
	 * PASSWORD VARCHAR(90) NOT NULL, USER_TYPE TINYINT NOT NULL, USER_STATUS TINYINT(2) NOT NULL, ADDITIONAL_ID
	 * VARCHAR(100), USER_CREATION_STAMP TIMESTAMP NOT NULL, USER_UPDATED_STAMP TIMESTAMP NOT NULL, USER_UPDATED_BY INT,
	 */
	...
	public List<UmsUser> getSystemUsers() throws DbException {
		DataSet dataSet = dbSession.executeQuery(SELECT_BY_TYPE_SYSTEM);
	...
	@Override
	public UmsUser getUserByUserAccessKey(String userAccessKey) throws DbException {
		DataSet dataSet = dbSession.executeQuery(SELECT_BY_USER_ACCESS_KEY, userAccessKey);
	...
	public UmsUser getUserByUsername(String userName) throws DbException {
		DataSet dataSet = dbSession.executeQuery(SELECT_BY_USERNAME, userName);
	...
	@Override
	public int updatePassword(String userAccessKey, String password) throws DbException {
		return dbSession.executeUpdate(UPDATE_PASSWORD_BY_USER_ACCESS_KEY, password, userAccessKey);
	}
	...
	public int insert(UmsUser user) throws DbException {
		return dbSession.executeUpdate(INSERT_ALL, user.getUserAccessKey(), user.getUsername(), user.getPassword(),
				user.getUserType().getUerTypeValue(), user.getUserStatus().getUserStatusValue(), user.getAdditionalId(),
				user.getUserCreationTime(), user.getUserUpdatedTime(), user.getUserUpdatedBy());
	}

	@Override
	public int update(UmsUser object) throws DbException {
		return dbSession.executeUpdate(SELECT_BY_USER_ACCESS_KEY, object.getUserAccessKey());
	}
	...
	public UmsUser getSingleRow(DataSet dataSet, int rowNum) {
		...
		user.setUserType(UserType.getUserType(dataSet.getIntValue(rowNum, "USER_TYPE")));
		user.setUserStatus(UserStatus.getUserStatus(dataSet.getIntValue(rowNum, "USER_STATUS")));
		user.setAdditionalId(dataSet.getStringValue(rowNum, "ADDITIONAL_ID"));
		user.setUserCreationTime(dataSet.getDateValue(rowNum, "USER_CREATION_STAMP"));
		user.setUserUpdatedTime(dataSet.getDateValue(rowNum, "USER_UPDATED_STAMP"));
	...
	public int softDelete(UmsUser umsUser) throws DbException {
		return dbSession.executeUpdate(DELETE_SOFT_BY_USER_ACCESS_KEY, umsUser.getUserAccessKey());
	}
	...
	public void assignCommunity(int communityId, int userId) throws DbException {
		dbSession.executeUpdate(ASSIGN_COMMUNITY, communityId, userId);
	}
